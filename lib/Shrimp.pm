...
=item [BugFix] If pid could not be grepped, it is tried again after 1 second
 and even if it cannot be determined, the program wont die. Canceling wont 
 work though.
...
sub run {
	...
	# timeout only makes sence if a pid as been determined
	return $self unless defined $self->{_pid}; 
...
sub _run_pipe_open {
	...
	if($self->{out}){
		...
		$V->exit("Failed to start Shrimp: $@") if $@;
		...
		#sleep(1);
		my $ps = qx(ps -eF | grep -v grep | grep $self->{bin} | grep X$open_pid);
		unless($ps){ # try again, if we were to fast
			sleep 1;
		...
		# NOTE: pid can still be undefined, e.g. if shrimp was to fast for ps
		
		# there does not have to be a pid if program terminated to fast for ps
		#  e.g. on failure or on --save on small files
		$V->exit("Childs pid not a number ".$self->{_pid}) if defined($self->{_pid}) && $self->{_pid} =~ /\D/;
	}
	...
	$V->exit("Couldn not find temporary error log: $tmp_error_log") unless -e $tmp_error_log;
