...
find(sub { if ( $File::Find::dir =~ /.+prime$/ ) { $alldirs{ $File::Find::dir } = 0; }},  $dir."/merged_ass" );

#herausfiltern der pfade bei denen keine contigverbindung assembliert werden konnte (vll nochmal mit velvet assembliern)
foreach (keys(%alldirs))
{
    unless ( -e $_."/extended_asc/scaffolds.fasta" )
    {
  	delete $alldirs{$_};
    }
}

print Dumper(%alldirs);
#exit();
